...
def create_filters(date=None, start_date=None, end_date=None,
    ...
    approaches that occured on exactly that given date.
    ...
    of `NEODatabase` because the main module directly passes this result
    to that method. For now, this can be thought of as a
    ...
    for key, value in locals().items():
        if value is not None:
            filters_list.append(DateFilter(operator.eq, value)) \
                if key == 'date' else None
            filters_list.append(DateFilter(operator.ge, value)) \
                if key == 'start_date' else None
            filters_list.append(DateFilter(operator.le, value)) \
                if key == 'end_date' else None
            filters_list.append(DistanceFilter(operator.ge, value)) \
                if key == 'distance_min' else None
            filters_list.append(DistanceFilter(operator.le, value)) \
                if key == 'distance_max' else None
            filters_list.append(VelocityFilter(operator.ge, value)) \
                if key == 'velocity_min' else None
            filters_list.append(VelocityFilter(operator.le, value)) \
                if key == 'velocity_max' else None
            filters_list.append(DiameterFilter(operator.ge, value)) \
                if key == 'diameter_min' else None
            filters_list.append(DiameterFilter(operator.le, value)) \
                if key == 'diameter_max' else None
            filters_list.append(HazardousFilter(operator.eq, value)) \
                if key == 'hazardous' else None
